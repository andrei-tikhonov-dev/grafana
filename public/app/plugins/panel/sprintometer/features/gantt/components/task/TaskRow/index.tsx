...
import { MTask, EViewMode, MTaskRowProps } from '../../../types';

/**
 * TaskRow Component - Displays and manages tasks for a single task group
 */
const TaskRow: React.FC<MTaskRowProps> = ({
  taskGroup,
  startDate,
  endDate,
  totalMonths,
  ...
  const [hoveredTask, setHoveredTask] = useState<MTask | null>(null);
  const [draggingTask, setDraggingTask] = useState<MTask | null>(null);
  const [dragType, setDragType] = useState<'move' | 'resize-left' | 'resize-right' | null>(null);
  const [dragStartX, setDragStartX] = useState(0);
  const [tooltipPosition, setTooltipPosition] = useState({ x: 0, y: 0 });
  const [previewTask, setPreviewTask] = useState<MTask | null>(null);
  ...
  const draggingTaskRef = useRef<MTask | null>(null);
  const previewTaskRef = useRef<MTask | null>(null);
  ...
  // Task update helpers
  const updateDraggingTask = (task: MTask) => {
    const taskCopy = {
      ...task,
  ...
  const updatePreviewTask = (task: MTask) => {
    const taskCopy = {
      ...task,
      startDate: new Date(task.startDate),
      endDate: new Date(task.endDate),
  ...
  const checkForAutoScroll = (clientX: number) => {
    if (!scrollContainerRef?.current || !draggingTask) {
      return;
    }
  ...
  const startAutoScroll = () => {
    ...
    const doScroll = () => {
      if (!autoScrollActive.current || !scrollContainerRef?.current || !targetPositionRef.current) {
        return;
      }
  ...
  const handleTaskClick = (_: React.MouseEvent, task: MTask) => {
  ...
  const handleTaskMouseEnter = (event: React.MouseEvent, task: MTask) => {
    if (!draggingTask) {
      setHoveredTask(task);
  ...
  const handleMouseDown = (event: React.MouseEvent, task: MTask, type: 'move' | 'resize-left' | 'resize-right') => {
    if (!editMode) {
      return;
    }
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent | MouseEvent) => {
    ...
    if (draggingTask && dragType && initialTaskState && rowRef.current && targetPositionRef.current) {
      try {
        ...
        if (viewMode === EViewMode.DAY && taskElementRef.current) {
          taskElementRef.current.style.left = `${newLeft}px`;
          taskElementRef.current.style.width = `${newWidth}px`;
          updateDatesFromPosition(newLeft, newWidth);
        }
  ...
  const finalizeTaskPosition = () => {
    if (!taskElementRef.current || !targetPositionRef.current || !draggingTaskRef.current) {
      return;
    }
  ...
  const handleProgressUpdate = (task: MTask, newPercent: number) => {
