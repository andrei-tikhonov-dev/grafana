...
import { MTaskGroup, EViewMode, MTaskListProps } from '../../types';

/**
 * TaskList Component - Displays the list of task groups on the left side of the Gantt chart
 */
const TaskList: React.FC<MTaskListProps> = ({
  tasks = [],
  ...
  const getGroupHeight = (taskGroup: MTaskGroup) => {
  ...
  const needsHierarchicalDisplay = viewMode === EViewMode.DAY || viewMode === EViewMode.WEEK;
