...
import { MTimelineProps, EViewMode } from '../../types';

/**
 * Timeline Component with hierarchical display for different view modes
 */
const Timeline: React.FC<MTimelineProps> = ({
  months,
  currentMonthIndex,
  locale = 'default',
  className = '',
  ...
  const getHigherLevelUnits = (): Array<{ date: Date; span: number }> => {
    if (![EViewMode.MINUTE, EViewMode.HOUR, EViewMode.DAY, EViewMode.WEEK].includes(viewMode) || months.length === 0) {
      return [];
    }

    const result: Array<{ date: Date; span: number }> = [];
  ...
  const needsHierarchicalDisplay = [EViewMode.MINUTE, EViewMode.HOUR, EViewMode.DAY, EViewMode.WEEK].includes(viewMode);
