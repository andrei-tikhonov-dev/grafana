...
import { MTask, EViewMode } from '../types';
...
export class TaskService {
  ...
  public static calculateDatesFromPosition(
    ...
    totalUnits: number,
    unitWidth: number,
    viewMode: EViewMode = EViewMode.MONTH
    ...
    try {
      ...
      switch (viewMode) {
        ...
        case EViewMode.DAY: { // Calculate how many units (days) from the start
   ...
   */
  public static createUpdatedTask(task: MTask, newStartDate: Date, newEndDate: Date): MTask {
    return {
      ...task,
  ...
  public static calculateTaskPixelPosition(
    ...
    try {
      ...
      } else if (viewMode === EViewMode.DAY) {
      ...
      const minWidthByViewMode = {
        [EViewMode.MINUTE]: 10, // Narrower for minute view
        [EViewMode.HOUR]: 15, // Narrow for hour view
        [EViewMode.DAY]: 20,
        [EViewMode.WEEK]: 20,
        [EViewMode.MONTH]: 20,
        [EViewMode.QUARTER]: 30,
        [EViewMode.YEAR]: 40,
      };
