...
const formLabelErrorStyles = css`
  &[data-error='true'] {
    color: ${theme2.colors.semantic.error};
  }
`;
...
function FormItem({ className, ...props }: React.ComponentProps<'div'>) {
  ...
  return (
    <FormItemContext.Provider value={{ id }}>
      <div data-slot="form-item" className={cx(formItemStyles, className)} {...props} />
...
function FormMessage({ className, ...props }: React.ComponentProps<'p'>) {
  ...
  return (
    <p data-slot="form-message" id={formMessageId} className={cx(formMessageStyles, className)} {...props}>
      {body}
    </p>
  );
}
