...
export function registerOutgoingDependenciesOptions(builder: any) {
  const showIf = (opts: TPanelOptions) => opts.panelType === EPanelType.OutgoingDependencies;

  builder.addSelect({
    path: 'outgoingDependencies.dashboard',
    name: 'Dashboard',
  ...
  builder.addSelect({
    path: 'outgoingDependencies.metric',
    name: 'Metric',
    category: DEFAULT_CONFIGURATION_CATEGORY,
    settings: {
