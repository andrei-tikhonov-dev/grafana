...
import { FetchResponse, getTemplateSrv, TemplateSrv, getBackendSrv } from '@grafana/runtime';
...
export class DataSource extends DataSourceApi<GrafanaQuery, GenericOptions> {
  ...
  private async initializeUserHeaders() {
    if (!this.currentUser) {
      try {
        const user = await getBackendSrv().get('/api/user');
        if (user && user.id) {
          this.currentUser = user;
          this.headers['x-grafana-user-id'] = user.id;
        } else {
  ...
  async query(options: QueryRequest): Promise<DataQueryResponse> {
    await this.initializeUserHeaders();
